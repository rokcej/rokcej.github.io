...
M3D.FullOrbitCameraControls = class{
    ...
    constructor(camera, orbitCenter){
        ...
        this._minPolarAngle = - Infinity; // radians
        this._maxPolarAngle = Infinity; // radians
    ...
    _getMouseMoveVector(mouse){
        let deltaMouseMove = new THREE.Vector3();
    ...
    _applyRotate(deltaMouseMove){
        ...
        let forwardVector = new THREE.Vector3(0, 0, -1).applyQuaternion(this._camera.quaternion);
    ...
    _applyOrbitalRotation(deltaMouseMove){
        ...
        let deltaRotQ = new THREE.Quaternion().setFromUnitVectors(orbitCenterToCameraVector, orbitCenterToCameraVector2);
    ...
    _applyTransform(inputData, deltaT){
        ...
        let clampMin = new THREE.Vector3(-1, -1, -1);
        let clampMax = new THREE.Vector3(1, 1, 1);
    ...
    return function (deltaT) {
        ...
        if (this._currentAnimation === null) {
            ...
            this._currentAnimation = {
                ...
                startPosQ: new THREE.Quaternion(),
                currentPosQ: new THREE.Quaternion(),
                endPosQ: new THREE.Quaternion().setFromUnitVectors(orbitCenterToCameraVector.clone().normalize(), orbitCenterToCameraVector2.clone().normalize()),
        ...
        if ( this._currentAnimation.timePassed < entry.time && (!this._currentAnimation.endPosQ.equals(this._currentAnimation.currentPosQ) || !entry.quaternion.equals(this._camera.quaternion)) ) {
            ...
            THREE.Quaternion.slerp( this._currentAnimation.startPosQ, this._currentAnimation.endPosQ, this._currentAnimation.currentPosQ, this._currentAnimation.timePassed / entry.time );
            ...
            //animate new rotation
            //a)
            //this._currentAnimation.startRot.slerp( entry.quaternion,  this._currentAnimation.timePassed / entry.time );
            //this._camera.quaternion = this._currentAnimation.startRot;
            //b)
            THREE.Quaternion.slerp( this._currentAnimation.startRot, entry.quaternion, this._camera.quaternion, this._currentAnimation.timePassed / entry.time );
