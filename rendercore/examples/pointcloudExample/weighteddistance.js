...
class App {
	...
	constructor(canvas) {
		...
		let LAS = new RC.LASLoader(this.loadingManager, "arraybuffer", true, 16*1024*1024);
		LAS.load(
			//url
			"../common/pointclouds/lesce-small-small.las",
			//set
			[
				RC.LASLoader.PDRFormat2.Keys.X, 
				RC.LASLoader.PDRFormat2.Keys.Y, 
				RC.LASLoader.PDRFormat2.Keys.Z, 
				RC.LASLoader.PDRFormat2.Keys.RED, 
				RC.LASLoader.PDRFormat2.Keys.GREEN, 
				RC.LASLoader.PDRFormat2.Keys.BLUE
			],
			//on load complete
			function(data){
			...
			function(data){
				...
				//if(points.length > data.size*3) points.splice(0, data.PDRs.size*3);
				//if(colors.length > data.size*4) colors.splice(0, data.PDRs.size*4);
				...
				pointCloudMaterial.pointSize = 800.0;
				window.App.pc = pointCloudMaterial;
		...
		this.camera.position = new RC.Vector3(1.6, 2.2, 35.5); //new RC.Vector3(15, 10, 80);//
		...
		var weightedDistanceMaterial = new RC.CustomShaderMaterial("weightedDistance");
		weightedDistanceMaterial.ligths = false;
